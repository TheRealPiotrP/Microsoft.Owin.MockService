...
namespace Microsoft.Kestrel.MockServer
{
    public class MockService : IDisposable
    {
        ...
        private readonly List<Tuple<Expression<Func<HttpRequest, bool>>, Func<HttpResponse, Task<HttpResponse>>>> _handlers;
        ...
        {
            _handlers = new List<Tuple<Expression<Func<HttpRequest, bool>>, Func<HttpResponse, Task<HttpResponse>>>>();
        ...
        internal MockService Setup(Expression<Func<HttpRequest, bool>> condition, Func<HttpResponse, Task<HttpResponse>> response)
        {
            _handlers.Add(new Tuple<Expression<Func<HttpRequest, bool>>, Func<HttpResponse, Task<HttpResponse>>>(condition, response));
        ...
        public async Task<HttpResponse> Invoke(HttpContext context)
        {
            try
            {
                foreach (var handler in _handlers)
                {
                    ...
                    return await handler.Item2(context.Response);
                }
        ...
        public void Dispose()
        {
            ...
            MockServiceRepository.Unregister(ServiceId);
